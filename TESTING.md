...
My Account |&check; | | [WAVE results for .html](docs/images/testing/wave/account.png)
All Services |  &check;| | [WAVE results for .html](docs/images/testing/wave/all-services.png)
Grass Cutting | &check; | | [WAVE results for .html](docs/images/testing/wave/grass.png)
Weeding |&check; | | [WAVE results for .html](docs/images/testing/wave/weed.png)
Tree Maintenance |&check; | | [WAVE results for .html](docs/images/testing/wave/tree.png)
Hedge Cutting |&check; | | [WAVE results for .html](docs/images/testing/wave/hedge.png)
Flowerbed Care |&check; | | [WAVE results for .html](docs/images/testing/wave/flower.png)
Tree Stump Removal |&check; | | [WAVE results for .html](docs/images/testing/wave/stump.png)
Reviews |&check; |  | [WAVE results for .html](docs/images/testing/wave/reviews.png)
Create Reviews |&check; |  | [WAVE results for .html](docs/images/testing/wave/leavereview.png)
Unpublished Reviews |&check; | | [WAVE results for .html](docs/images/testing/wave/unpublished.png)
Cart |&check; | | [WAVE results for .html](docs/images/testing/wave/cart.png)
Checkout |&check; | One acceptable alert. See above. | [WAVE results for .html](docs/images/testing/wave/checkout.png)
Checkout Success |&check; | | [WAVE results for .html](docs/images/testing/wave/checkout-success.png)
Contact Us |&check; | | [WAVE results for .html](docs/images/testing/wave/contact.png)
Newsletter | &check; | One acceptable error. See comment above. | [WAVE results for .html](docs/images/testing/wave/newsletter.png)
Privacy Policy |&check; | | [WAVE results for .html](docs/images/testing/wave/privacy.png)
403 |&check;| | [WAVE results for .html](docs/images/testing/wave/403.png)
404 |&check; | | [WAVE results for .html](docs/images/testing/wave/404.png)
500 |&check; | | [WAVE results for .html](docs/images/testing/wave/500.png)
Sign In |&check; | | [WAVE results for .html](docs/images/testing/wave/login.png)
Sign Up |&check; | | [WAVE results for .html](docs/images/testing/wave/signup.png)
Sign Out |&check; | | [WAVE results for .html](docs/images/testing/wave/logout.png)
Reset Password |&check; | | [WAVE results for .html](docs/images/testing/wave/password-reset.png)
Change Password |&check; | | [WAVE results for .html](docs/images/testing/wave/change-password.png)
Sign Up Email Sent | &check;| | [WAVE results for .html](docs/images/testing/wave/confirm-email.png)
...
11-b | After solving Bug 11-a it was noticed that the problem happened to all services, i.e. the attributes get removed from the cart dictionary. || The issue was resolved by auditing the variable names being used in the `amend_cart` view and correcting them so that instead of progressing to the last else block thinking that it was safe to overwrite existing cart data, the view instead updated the number of a specfic service without affecting other entries in the dictionary.
12 | When submitting an order containing a tree service, the site returned a 500 error with a `RelatedObjectDoesNotExist` exception.  The OrderLineItem model did not seem to have a value for the service field. This was not an issue with any other type of service on the website. The error only arose when calculating the line item total. | ![Debug page - bug #12](docs/images/testing/bugs/bug-12.png) | In the view, the value of `service` was not being returned in the context for this specific case. The line was removed previously during debugging and not replaced. After adding it back, the issue was resolved.
...
21 | When the user has saved their details for later, and the form is autopopulated with their personal and location information, the "Continue to Payment" button is disabled until the user makes a changes in one of the required fields. | ![Checkout autofilled - bug #21](docs/images/testing/bugs/bug-21.png) | The JS code to enable or disable the button was only being called on form field keyup events. This code was wrapped in a function and then called both on page load and on keyup events for the form fields.
...
23 |When adding an event listener to the review form submit button in order to use custom validation for the star ratings, the jQuery listener could not be triggered. The console log statement pictured here was not printed however calling one from a click event listener on the button did print to the console.| ![Form submit - bug #23](docs/images/testing/bugs/bug-23.png) | The cause of the issue was that the default browser validation was preventing the submit event from even occurring in the first place. It was necessary to disable the default form validation by adding `novalidate` and handling all the form validation in the JavaScript file.
...
35 |When editing a review and saving the changes, an Uncaught RangeError message was displayed in the console; however, the review changes did save correctly to the databse. | ![Uncaught RangeError - bug #35](docs/images/testing/bugs/bug-35.png) | After reading a [StackOverflow post](https://stackoverflow.com/questions/7658775/chrome-jquery-uncaught-rangeerror-maximum-call-stack-size-exceeded) that asks about the same error, it was realised that a recursive loop had inadvertenly been created in the custom form validation block in `review_stars.js`. An event listener for the form submit event was added which called `preventDefault()` if any field was empty. If no fields were empty, the form was submitted. This caused the submit event listener functions to run recursively over and over! To solve the bug, the submit event listener was changed to a click event listener for the button element, and the error messages no longer displayed in the console.
